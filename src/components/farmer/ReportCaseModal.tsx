  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-md overflow-y-auto animate-in fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden my-6 flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="bg-emerald-500/10 text-emerald-400 text-[10px] font-extrabold uppercase px-2 py-0.5 rounded border border-emerald-500/30">
        ...
        <div className="px-6 pt-3 pb-2 bg-slate-950/40 border-b border-slate-800/60">
          <div className="flex items-center justify-between mb-1.5 text-[11px] font-semibold text-slate-400">
            ...
            <span className="text-emerald-400">{stepTitles[currentStep - 1]}</span>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-5">
          {validationError && (
            <div className="bg-rose-950/80 border border-rose-600/80 rounded-xl p-3 text-xs text-rose-200 flex items-center gap-2 animate-in shake">
            ...
            <div className="space-y-4 animate-in fade-in">
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 rounded-2xl border text-center transition-all ${
                          isSelected
                            ? 'bg-emerald-600/20 border-emerald-400 text-white ring-1 ring-emerald-400'
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2">
                  ...
                  <p className="text-[11px] text-slate-400">Total count of this species owned</p>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                ...
                <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2">
                  ...
                  <p className="text-[11px] text-slate-400">Showing symptoms currently</p>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
          ...
          {currentStep === 2 && (
            <div className="space-y-4 animate-in fade-in">
                    ...
                    return (
                      <button
                        ...
                        className={`p-3 rounded-xl border text-left text-xs transition-all flex items-center justify-between gap-2 ${
                          isSelected
                            ? 'bg-emerald-600/20 border-emerald-400 text-emerald-300 font-semibold'
                            : 'bg-slate-950 border-slate-800 text-slate-300 hover:border-slate-700'
                        }`}
                      >
                        <span>{sym}</span>
              ...
              <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800 space-y-2">
                <label className="text-xs font-bold text-slate-400">Other Specific Symptom (Optional)</label>
          ...
          {currentStep === 3 && (
            <div className="space-y-4 animate-in fade-in">
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2">
                ...
                <p className="text-xs text-slate-400">Mortality is a primary indicator for emergency veterinary dispatch</p>
          ...
          {currentStep === 4 && (
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                <span className="text-xs font-bold text-slate-300 block">Lesion Photograph (Mouth, Hoof, or Skin)</span>
                ...
                {isCameraActive && (
                  <div className="space-y-2 bg-black rounded-xl p-2 border border-slate-800">
                    <div className="relative aspect-video rounded-lg overflow-hidden bg-slate-900 flex items-center justify-center">
                      ...
                      {/* Reticle Overlay */}
                      <div className="absolute inset-0 border-2 border-dashed border-emerald-400/60 rounded m-6 pointer-events-none flex items-center justify-center">
                        <span className="text-[10px] text-emerald-300 font-mono bg-slate-950/70 px-2 py-0.5 rounded">
                          Target Lesion
                        </span>
                      </div>
                    </div>
                ...
                {capturedSnapshot && (
                  <div className="space-y-2 bg-slate-900 p-3 rounded-xl border border-slate-800">
                    <span className="text-xs text-emerald-400 font-bold block">Snapshot Captured:</span>
                    <img src={capturedSnapshot} alt="Snapshot" className="h-36 rounded-lg object-cover mx-auto border border-slate-700" />
                ...
                {!isCameraActive && !capturedSnapshot && photoUrl && (
                  <div className="flex items-center gap-3 bg-slate-900 p-2.5 rounded-xl border border-slate-800">
                    <img src={photoUrl} alt="Attached Lesion" className="w-14 h-14 rounded-lg object-cover border border-slate-700 shrink-0" />
                    <div className="flex-1 min-w-0">
                      <span className="text-xs font-bold text-white block truncate">{photoFileName}</span>
                      <span className="text-[11px] text-emerald-400 font-semibold">✓ Attached for Veterinary Inspection</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-slate-300 block">Voice Description (Speech-to-Text)</span>
                    <span className="text-[11px] text-slate-400">Speak in {lang === 'te' ? 'Telugu (తెలుగు)' : lang === 'hi' ? 'Hindi (हिंदी)' : 'English'}</span>
                  </div>
                ...
                {isRecording && (
                  <p className="text-xs text-amber-400 font-semibold animate-pulse">{recordingStatus}</p>
                )}
                ...
                <div className="space-y-1.5">
                  ...
                  {isEditingTranscript ? (
                    ...
                    <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 text-xs text-slate-200 leading-relaxed italic">
          ...
          {currentStep === 5 && (
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
                <div>
                  <label className="text-xs font-bold text-slate-300 block mb-1">Village & Mandal</label>
                  <select
                    value={village}
                ...
                <div className="pt-2 border-t border-slate-800 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-400">GPS Geo-Coordinates</span>
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-slate-900 p-2.5 rounded-xl border border-slate-800 text-xs">
                      ...
                      <span className="font-mono font-bold text-white">{coords.lat}° N</span>
                    </div>
                    <div className="bg-slate-900 p-2.5 rounded-xl border border-slate-800 text-xs">
                      <span className="text-[10px] text-slate-400 block uppercase">Longitude</span>
                      <span className="font-mono font-bold text-white">{coords.lng}° E</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3 text-xs">
                <div className="grid grid-cols-2 gap-2 border-b border-slate-800 pb-3">
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase block">Species & Herd</span>
                    <span className="font-bold text-white">{animalType} ({totalAnimals} total)</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase block">Morbidity / Mortality</span>
                    <span className="font-bold text-amber-400">{sickCount} Sick, {deadCount} Dead</span>
                  </div>
                </div>

                <div>
                  <span className="text-[10px] text-slate-400 uppercase block mb-1">Symptoms Checklist</span>
                  <div className="flex flex-wrap gap-1">
                    {selectedSymptoms.map(s => (
                      <span key={s} className="bg-slate-900 border border-slate-800 text-emerald-300 text-[11px] px-2 py-0.5 rounded">
                        {s}
                      </span>
                    ))}
                  </div>
                ...
                <div className="border-t border-slate-800 pt-3">
                  <span className="text-[10px] text-slate-400 uppercase block mb-0.5">Location & Voice Evidence</span>
                  <p className="text-slate-300">📍 {village} ({coords.lat}° N, {coords.lng}° E)</p>
                  <p className="text-slate-400 italic text-[11px] mt-1">"{voiceTranscript}"</p>
                </div>
              </div>

              <div className="bg-emerald-950/40 border border-emerald-800/40 p-3.5 rounded-2xl flex items-start gap-2.5">
                <input
                  type="checkbox"
                  id="declaration"
                  checked={declarationConfirmed}
                  onChange={(e) => setDeclarationConfirmed(e.target.checked)}
                  className="mt-0.5 w-4 h-4 rounded border-slate-700 text-emerald-600 focus:ring-emerald-500"
                />
                <label htmlFor="declaration" className="text-xs text-emerald-200 leading-snug cursor-pointer">
          ...
          {currentStep === 7 && submittedCase && (
            <div className="space-y-5 animate-in zoom-in-95">
              <div className="text-center space-y-2 py-2">
                <div className="w-14 h-14 rounded-full bg-emerald-600/20 border-2 border-emerald-500/50 flex items-center justify-center text-2xl mx-auto text-emerald-400 shadow-xl">
                  ✓
                </div>
              ...
              <div className={`p-4 rounded-2xl border space-y-3 ${
                submittedCase.riskLevel === 'HIGH'
                  ...
                  : 'bg-emerald-950/40 border-emerald-600/60 shadow-lg shadow-emerald-950/30'
              }`}>
                <div className="flex items-center justify-between">
                ...
                <div className="text-xs text-slate-200 space-y-1 pt-1">
                  <p><b>Recommended Action:</b> <span className="text-emerald-300 font-semibold">{submittedCase.recommendedAction}</span></p>
                  <p className="text-slate-400"><b>Primary Symptoms:</b> {submittedCase.symptoms.join(', ')}</p>
                </div>

                <div className="pt-2 border-t border-slate-800/80 space-y-1">
                  <span className="text-[11px] font-bold text-slate-400 uppercase block">Algorithmic Factors:</span>
                ...
                <div className="bg-slate-950/80 p-3 rounded-xl border border-slate-800 text-[11px] text-slate-400 leading-relaxed flex items-start gap-2">
